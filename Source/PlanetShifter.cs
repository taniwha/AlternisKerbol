...
//        private float[] newWarps = { 1, 10, 50, 100, 1000, 10000, 100000, 1000000 };
        ...
        //Textures
        public Texture2D newMinmusTexture;
...
*/
            ...
            if (HighLogic.LoadedScene == GameScenes.MAINMENU && !alternisDone) //make extra sure something hasn't goofed and tried to run this elsewhere
            {
                #region textures
                //Load scaled maps
                newMinmusTexture = GameDatabase.Instance.GetTexture(path + "redminmus_dif", false);
                if (newMinmusTexture != null)
                    print("New minmus texture loaded");
                ...
                foreach (CelestialBody cb in FlightGlobals.Bodies)
                {
                    ...
                    {
                        cb_sun = cb;
                        print("PlanetShifter: Found Sun");
                    }

                    if (cb.gameObject.name == "Moho")
                    {
                        cb_moho = cb;
                        print("PlanetShifter: Found Moho");
                    }

                    if (cb.gameObject.name == "Eve")
                    {
                        cb_eve = cb;
                        print("PlanetShifter: Found Eve");
                    }

                    if (cb.gameObject.name == "Gilly")
                    {
                        cb_gilly = cb;
                        print("PlanetShifter: Found Gilly");
                    }

                    if (cb.gameObject.name == "Kerbin")
                    {
                        cb_kerbin = cb;
                        print("PlanetShifter: Found Krebin"); //yes, krebin
                    }

                    if (cb.gameObject.name == "Duna")
                    {
                        cb_duna = cb;
                        print("PlanetShifter: Found Duna");
                    }

                    if (cb.gameObject.name == "Ike")
                    {
                        cb_ike = cb;
                        print("Shifter: Found Ike");
                    }

                    if (cb.gameObject.name == "Mun")
                    {
                        cb_mun = cb;
                        print("PlanetShifter: Found Mun");
                    }

                    if (cb.gameObject.name == "Minmus")
                    {
                        cb_minmus = cb;
                        print("PlanetShifter: Found Minmus");
                    }

                    if (cb.gameObject.name == "Jool")
                    {
                        cb_jool = cb;
                        print("PlanetShifter: Found Jool");
                    }

                    if (cb.gameObject.name == "Tylo")
                    {
                        cb_tylo = cb;
                        print("PlanetShifter: Found Tylo");
                    }

                    if (cb.gameObject.name == "Laythe")
                    {
                        cb_laythe = cb;
                        print("PlanetShifter: Found Laythe");
                    }

                    if (cb.gameObject.name == "Vall")
                    {
                        cb_vall = cb;
                        print("PlanetShifter: Found Vall");
                    }

                    if (cb.gameObject.name == "Bop")
                    {
                        cb_bop = cb;
                        print("PlanetShifter: Found Bop");
                    }

                    if (cb.gameObject.name == "Pol")
                    {
                        cb_pol = cb;
                        print("PlanetShifter: Found Pol");
                    }

                    if (cb.gameObject.name == "Dres")
                    {
                        cb_dres = cb;
                        print("PlanetShifter: Found Dres");
                    }

                    if (cb.gameObject.name == "Eeloo")
                    {
                        cb_eeloo = cb;
                        print("PlanetShifter: Found Eeloo");
                    }
                }
                #endregion

                //Do things to planets, but make sure all of them are present and accounted for first...
                //This feels like a wrong way to do things but it's done only once, so...
                ...
                cb_bop != null && cb_pol != null && cb_laythe != null && cb_dres != null && cb_eeloo != null)
                {
                    ...
                    cb_moho.orbit.semiMajorAxis = 4075040399;
                    cb_moho.orbit.inclination = 0.2502;
                    cb_moho.orbit.eccentricity = 0.3;
                    ...
                    cb_eve.orbit.semiMajorAxis = 8185550300;
                    cb_eve.orbit.inclination = 1.02;
                    ...
                    cb_minmus.orbitDriver.orbitColor = new Color(0.3f, 0.1f, 0.1f);
                    cb_minmus.Radius = 29000;
                    cb_minmus.pqsController.radius = 29000;
                    cb_minmus.tidallyLocked = true;
                    ...
                    //Do things to minmus' terrain
                    PQSMod_VertexPlanet cb_minmus_vp = cb_minmus.transform.GetComponentInChildren<PQSMod_VertexPlanet>();
                    if (cb_minmus_vp != null)
                    {
                        print("PlanetShifter: Shifting Minmus Terrain");
                        cb_minmus_vp.deformity = 2450; //less than half, since that was the original
                        foreach (PQSMod_VertexPlanet.LandClass l in cb_minmus_vp.landClasses)
                        {
                            if (l.name == "AbyPl")
                            {
                                l.baseColor = new Color(0.376f, 0.141f, 0.117f, 1.000f);
                                l.colorNoise = new Color(0.386f, 0.151f, 0.127f, 1.000f);
                            }
                            if (l.name == "Beach")
                            {
                                l.baseColor = new Color(0.611f, 0.260f, 0.220f, 1.000f);
                                l.colorNoise = new Color(0.500f, 0.250f, 0.230f, 1.000f);
                            }
                            if (l.name == "Snow" || l.name == "Grass")
                            {
                                l.baseColor = new Color(0.198f, 0.128f, 0.062f, 1.000f);
                                l.colorNoise = new Color(0.18f, 0.12f, 0.05f, 1.000f);
                            }
                        }

                        if (cb_minmus_vp.gameObject.GetComponent<PQSMod_VoronoiCraters>() == null)
                        {
                            PQSMod_VoronoiCraters cb_minmus_craters = cb_minmus_vp.gameObject.AddComponent<PQSMod_VoronoiCraters>();
                            if (cb_minmus_craters != null && muncraters != null)
                            {
                                print("...and giving it craters");
                                cb_minmus_craters.voronoiFrequency = 6;
                                cb_minmus_craters.deformation = 200;
                                cb_minmus_craters.craterCurve = new AnimationCurve(muncraters.craterCurve.keys);
                                cb_minmus_craters.craterColourRamp = FetchSolidGradient(new Color(0.386f, 0.151f, 0.127f, 1.000f));
                                cb_minmus_craters.jitter = 0.1f;
                                cb_minmus_craters.jitterCurve = new AnimationCurve(muncraters.jitterCurve.keys);
                                cb_minmus_craters.jitterHeight = 3;
                                cb_minmus_craters.rOffset = 1;
                                cb_minmus_craters.rFactor = 0.5f;
                                cb_minmus_craters.voronoiDisplacement = 0.1f;
                                cb_minmus_craters.order = cb_minmus_vp.order + 1;
                            }
                        }
                    }

                    /*print("Dissecting Minmus VertexPlanet");
                    print("Def: "+vp_minmus.deformity);

                    foreach (PQSMod_VertexPlanet.LandClass l in vp_minmus.landClasses)
                    {
                        print("Found landclass "+l.name);
                        print("baseColor: "+l.baseColor);
                        print("colorNoise: " + l.colorNoise);
                    }*/
                    ...
                    cb_dres.orbit.semiMajorAxis = 17580043;
                    cb_dres.orbit.eccentricity = 0.002;
                    cb_dres.orbit.inclination = 0.109;
                    ...
                    {
                        cb_bop.orbit.eccentricity = 0.09;
                        cb_bop.orbit.inclination = 0.894;
                        cb_bop.orbit.semiMajorAxis = 200805430;
                    ...
                    cb_sun.orbitingBodies.Remove(cb_dres);
                    cb_sun.orbitingBodies.Remove(cb_duna);
                    cb_jool.orbitingBodies.Add(cb_kerbin);
                    cb_jool.orbitingBodies.Add(cb_minmus);
                    cb_jool.orbitingBodies.Add(cb_mun);
                    cb_tylo.orbitingBodies.Add(cb_duna);
                    cb_tylo.orbitingBodies.Add(cb_dres);
                    cb_eeloo.orbitingBodies.Add(cb_vall);
                    ...
                    cb_dres.orbit.referenceBody = cb_tylo;
                    cb_vall.orbit.referenceBody = cb_eeloo;
                    cb_minmus.orbit.referenceBody = cb_jool;
                    ...
                    foreach (CelestialBody cb2 in FlightGlobals.Bodies)
                    {
                        if (cb2.gameObject.name != "Sun")
                        {
                            ...
                            if (cb2.gameObject.name == "Kerbin") cb2.orbit.period = 211926; //2 days 10 hours 52 minutes 6 seconds
                            else if (cb2.gameObject.name == "Mun") cb2.orbit.period = 105963; //1 day 5 hours 26 minutes 3 seconds
                    ...
                    alternisDone = true; //and stay out
     
                }
            }
        }
        #region dumb gradient thing for making craters solid color
        Gradient FetchSolidGradient(Color c)
        {
